...
| Play mode (F9) com física e restauração das transformações | Parcial | o editor e a engine rodam dois loops de render na mesma cena (Planejado: F2) |
...
| Autosave | Parcial | uma única chave no localStorage para todos os projetos; restaurar duplica a cena (Planejado: F2) |
| Aba Terrain: esculpir, pintar, erosão em worker, água | Parcial | o worker de erosão não funciona no build de produção (Planejado: F2) |
...
| Aba Blueprints (grafo visual) | Parcial | 75 dos 154 nós têm implementação; só roda pelo botão Simulate, não em play (Planejado: F4) |
...
| ECS (World, Entity, Component, System) | Funciona | cache de arquétipo e budgets por sistema são cosméticos (Planejado: F3) |
| Física Rapier 3D: corpos, colisores, character controller, raycast, joints | Parcial | passo variável (depende do FPS); contatos com terreno não geram eventos (Planejado: F2) |
...
| Feature | Status | Notas |
|---|---|---|
| Servidor de salas (Express + ws) | Parcial | uma mensagem malformada derruba o processo; sem validação nem limites (Planejado: F2) |
| Cliente de rede (`NetworkManager`) | Parcial | não é usado pelo editor nem pelos templates (Planejado: F9) |
